...
namespace Microsoft.Solutions.PatientHub.RealtimeInferenceService
{
    public class RealtimeInference
    {
        ...
        public async Task<Prediction> GetTop5RealtimeInference(ColumnNameMapService columnNameMapService, ColumnLookupValueService columnLookupValueService,PatientData patientData)
        {
            ...
            //retira de dentro da lista
            string resultado1 = jsonRequestParam.Substring(1, jsonRequestParam.Length - 2);

                                  // Converte a string em um dicionário
                var dict = JsonConvert.DeserializeObject<Dictionary<string, object>>(resultado1);
                Dictionary<string, object> meuDicionario = new Dictionary<string, object>();
                ...
                {   
                    ...
                    aux["1"] = entry.Value;
                    meuDicionario.Add(entry.Key, aux);
                }

        

        string json = JsonConvert.SerializeObject(meuDicionario, new JsonSerializerSettings { Formatting = Formatting.None });
