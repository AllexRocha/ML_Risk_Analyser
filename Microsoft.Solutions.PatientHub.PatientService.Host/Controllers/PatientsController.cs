...
namespace Microsoft.Solutions.PatientHub.PatientService.Host.Controllers
{
    public class PatientsController : Controller
    {
        ...
        [HttpPut]
        [Route("Patients/{PatientID}")]
        public async Task<ActionResult<Patient>> UpdatePatient(string PatientID, [FromBody]BasicUpdatePatient patient)
        {
            var result = await _patientService.BasicUpdate(PatientID, patient);
            return (result is not null) ? Ok(result) : NotFound();
        }
